...
# Corrige conflito com brp-compress/manpages
rm -rf %{buildroot}%{_mandir}/man1 || true
rm -rf %{buildroot}%{_mandir}/man3 || true
rm -rf %{buildroot}%{_mandir}/man5 || true
rm -rf %{buildroot}%{_mandir}/man7 || true
rm -rf %{buildroot}%{_mandir}/man8 || true
...
%changelog
* Sun Apr 12 2026 Guilherme Suzin <gui.suzin@live.com> - 4.24.0-1
- Update to Samba 4.24.0
- First packaging attempt for Samba 4.24 in the new GRX Solutions COPR repository
- Keep legacy single-package layout compatible with previous successful COPR workflow
- Preserve manual manpage cleanup during %%install to avoid manifest and brp-compress conflicts
- Preserve embedded AD DC systemd unit and default smb.conf bootstrap
- Includes 4.24 security and authentication-related improvements:
- Added authentication information audit support for altSecurityIdentities, dNSHostName, msDS-AdditionalDnsHostName, msDS-KeyCredentialLink and servicePrincipalName
- Added support for remote password management policy hints for Entra ID SSPR and Keycloak
- Added Kerberos PKINIT KeyTrust logon support
- Added validation for msDS-KeyCredentialLink updates
- Added strong and flexible certificate key mappings for PKINIT
- Added support for PKINIT SID extension
- KDC now includes PAC by default
- Added kdc require canonicalization support
- Domain encryption types now default to AES for supported domain functional levels

* Sun Apr 12 2026 Guilherme Suzin <gui.suzin@live.com> - 4.23.6-2
- Retry build for new GRX COPR repository
- Keep legacy single-package layout for compatibility with previous successful COPR workflow
- Keep manual manpage cleanup during %%install to avoid brp-compress conflicts
- Preserve Samba AD DC systemd unit and default smb.conf bootstrap
- Rebuild package for validation in clean COPR chroot

* Sat Mar 07 2026 Guilherme Suzin <suporte@onbit.tech> - 4.23.6-1
- Update to Samba 4.23.6
- Keep single-package layout compatible with existing COPR workflow
- Added --localstatedir=/var to fix /usr/var/lib/samba path issue
- Fixed ghost path for /etc/samba/smb.conf
- Use parallel make with %%{?_smp_mflags}
- Removed installed manpages during %%install to avoid brp-compress manifest conflicts
- Package revision 4.23.6_03

* Thu Nov 20 2025 Guilherme Suzin <suporte@onbit.tech> - 4.23.3-9
